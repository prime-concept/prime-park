...
final class ConciergeTableViewCell: UITableViewCell {
    ...
    func setData(item: Issue) {
        ...
        timeLabel.text = dateFormatter.string(from: item.createdAt)
        descriptionLabel.text = item.descriptionText
        dateAndGuestsLabel.text = item.createDateDescription
        //"\(item.createDateDescription) · \(item.guestsDescription)"
